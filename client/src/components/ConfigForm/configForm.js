...
function ConfigForm({ parameters, config, updateConfig = () => { }, children }) {
    ...
    const form = (
        <div className="w-full flex flex-wrap">
            <div className="w-1/3 mr-10 h-36">
                {children}
            </div>
            {parameters == null ? '' :
                parameters.map((parameter, i) => {
                    if (parameter.inputType === 'number' && parameter.type === 'config') {
                        return (
                            <div className="w-1/3 mr-10 h-36" key={"single-line-text-" + parameter.name}>
                                <SingleLineText
                                    name={parameter.name}
                        ...
                        return (
                            <div className="w-4/5" key={"multiSelect-Config-" + parameter.name}>
